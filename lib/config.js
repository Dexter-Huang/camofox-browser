...
function camoufoxCacheDir(env = process.env) {
  ...
  if (process.platform === "darwin")
    return join(home, "Library", "Caches", "camoufox");
...
function readBoundedInteger(value, fallback, { minimum = 0 } = {}) {
...
function loadConfig({ configPath = CONFIG_PATH } = {}) {
  ...
  const browserIdleTimeoutMs = parseInt(
  ...
  const newPageTimeoutMs =
    Number.isFinite(configuredNewPageTimeoutMs) &&
    configuredNewPageTimeoutMs > 0
      ? configuredNewPageTimeoutMs
      : 10000;
  const configuredInteractiveMode = fileConfig.interactive?.mode;
  const interactiveMode = normalizeInteractiveMode(
    process.env.CAMOFOX_INTERACTIVE || configuredInteractiveMode,
  );
  return {
    ...
    uploadsDir:
      process.env.CAMOFOX_UPLOADS_DIR ||
      join(os.homedir(), ".camofox", "uploads"),
    profileDir:
    ...
    browserIdleTimeoutMs: Number.isFinite(browserIdleTimeoutMs)
      ? browserIdleTimeoutMs
      : 300000,
    nativeMemRestartThresholdMb:
      parseInt(process.env.NATIVE_MEM_RESTART_THRESHOLD_MB) || 300,
    browserRssRestartThresholdMb:
      parseInt(process.env.BROWSER_RSS_RESTART_THRESHOLD_MB) || 1500,
    ...
    interactiveMode,
    // 截图 PNG 会占用 Node 外部内存。此处预算由 server.js 的统一协调器执行，
    // 不允许各接口自行并发 page.screenshot()。
    screenshotIntervalMs: readBoundedInteger(
      process.env.CAMOFOX_SCREENSHOT_INTERVAL_MS,
      500,
    ),
    screenshotMaxConcurrent: readBoundedInteger(
      process.env.CAMOFOX_SCREENSHOT_MAX_CONCURRENT,
      2,
      { minimum: 1 },
    ),
    screenshotMaxQueued: readBoundedInteger(
      process.env.CAMOFOX_SCREENSHOT_MAX_QUEUED,
      16,
    ),
    screenshotMaxQueuedPerTab: readBoundedInteger(
      process.env.CAMOFOX_SCREENSHOT_MAX_QUEUED_PER_TAB,
      3,
      { minimum: 1 },
    ),
    screenshotCacheMaxBytes: readBoundedInteger(
      process.env.CAMOFOX_SCREENSHOT_CACHE_MAX_BYTES,
      32 * 1024 * 1024,
      { minimum: 1 },
    ),
    proxy: {
    ...
    serverEnv: {
      ...
      CAMOFOX_EVALUATE_MAX_BODY_SIZE:
        process.env.CAMOFOX_EVALUATE_MAX_BODY_SIZE,
      ...
      CAMOFOX_DISABLE_DEFAULT_ADDONS:
        process.env.CAMOFOX_DISABLE_DEFAULT_ADDONS,
      CAMOFOX_INTERACTIVE: process.env.CAMOFOX_INTERACTIVE,
      CAMOFOX_SCREENSHOT_INTERVAL_MS:
        process.env.CAMOFOX_SCREENSHOT_INTERVAL_MS,
      CAMOFOX_SCREENSHOT_MAX_CONCURRENT:
        process.env.CAMOFOX_SCREENSHOT_MAX_CONCURRENT,
      CAMOFOX_SCREENSHOT_MAX_QUEUED: process.env.CAMOFOX_SCREENSHOT_MAX_QUEUED,
      CAMOFOX_SCREENSHOT_MAX_QUEUED_PER_TAB:
        process.env.CAMOFOX_SCREENSHOT_MAX_QUEUED_PER_TAB,
      CAMOFOX_SCREENSHOT_CACHE_MAX_BYTES:
        process.env.CAMOFOX_SCREENSHOT_CACHE_MAX_BYTES,
