...
import {
  ...
  setupExpressErrorHandler as setupSentryErrorHandler,
...
const app = express();
// 所有 HTTP 截图端点共用这个协调器。PNG Buffer 位于 Node 外部内存，不能依赖
// 单个路由的限流来防止多接口同时调用 page.screenshot() 造成进程 OOM。
const screenshotBroker = createScreenshotBroker({
  intervalMs: CONFIG.screenshotIntervalMs,
  maxConcurrent: CONFIG.screenshotMaxConcurrent,
  maxQueued: CONFIG.screenshotMaxQueued,
  maxQueuedPerTab: CONFIG.screenshotMaxQueuedPerTab,
  maxCacheBytes: CONFIG.screenshotCacheMaxBytes,
});
app.locals.screenshotBroker = screenshotBroker;

function captureTabScreenshot({ userId, tabId, page, fullPage = false }) {
  return screenshotBroker.capture({
    tabKey: `${normalizeUserId(userId)}:${tabId}`,
    variant: fullPage ? "page:full" : "page:viewport",
    capture: () => page.screenshot({ type: "png", fullPage }),
  });
}

function captureTabScreenshotBase64({ userId, tabId, page }) {
  return screenshotBroker.captureBase64({
    tabKey: `${normalizeUserId(userId)}:${tabId}`,
    variant: "page:viewport",
    capture: () => page.screenshot({ type: "png", fullPage: false }),
  });
}
...
const SKIP_PATTERNS = [/datepicker/i, /date.?picker/i, /calendar/i, /^date$/i];
...
function selectorValidationError(selector) {
  ...
  if (/^text\s*\(/i.test(selector))
    return "Invalid text selector syntax. Use text=... or a valid CSS selector.";
  const stack = [];
  let quote = null;
  let escaped = false;
  const pairs = { ")": "(", "]": "[", "}": "{" };
  for (const ch of selector) {
    ...
    if (ch === ")" || ch === "]" || ch === "}") {
      if (stack.pop() !== pairs[ch])
        return `Invalid selector syntax: ${selector}`;
    }
  }
...
async function assertLocatorFillable(locator) {
  ...
  if (info.tagName === "input" && !NON_FILLABLE_INPUT_TYPES.has(info.type))
    return;
  if (info.role === "textbox" || info.role === "searchbox") return;
  const target = info.type
    ? `${info.tagName}[type=${info.type}]`
    : info.role
      ? `${info.tagName}[role=${info.role}]`
      : info.tagName || "element";
  throw Object.assign(
    new Error(
      `Element ${target} is not fillable. Use click for buttons and other controls.`,
    ),
    {
...
async function safePageClose(page) {
  ...
  try {
    ...
    log("warn", "page close timed out or failed, force-closing", {
      error: e.message,
    });
    try {
      await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(
            () => reject(new Error("page force-close timed out")),
            PAGE_FORCE_CLOSE_TIMEOUT_MS,
          ),
        ),
      ]);
    } catch (_) {}
  ...
  browserWarmRetryTimer = setTimeout(async () => {
    ...
    try {
      ...
      if (isFatalInstallError(err)) {
        log(
          "error",
          "browser unavailable: Camoufox binaries are not installed; aborting retry loop",
          {
            error: err.message,
...
async function recoverUserSession(userId, reason) {
  ...
  log("warn", "recovering user session after nav failure threshold", {
    userId: key,
    reason,
    failures: getUserNavHealth(key).consecutiveNavFailures,
  });
...
async function restartBrowser(reason) {
  ...
  const totalFailures = Array.from(userNavHealth.values()).reduce(
...
function getExternalCamoufoxLaunch() {
  ...
  if (!externalCamoufoxLaunch) {
    externalCamoufoxLaunch = prepareExternalCamoufoxExecutable(
      CONFIG.camoufoxExecutablePath,
      {
...
async function _forceKillBrowserProcesses(reason, ownedBrowserProcesses = []) {
  ...
  let victims = [];
  try {
    victims = survivingOwnedBrowserProcesses(ownedBrowserProcesses).map(
      (proc) => proc.pid,
    );
  } catch (err) {
    log("warn", "failed to scan for browser survivor processes", {
      reason,
      error: err.message,
    });
    return;
...
async function createPageWithRecoveryForUser(
  userId,
  session,
  { trace = false } = {},
) {
...
function handleRouteError(err, req, res, extraFields = {}) {
  ...
  if (isPageCrashedError(err)) {
    if (foundForContext)
      destroyTab(
        sessions.get(normalizeUserId(userId)),
        tabId,
        "page_crashed",
        userId,
      );
    return res.status(410).json({
      error: "Page crashed. Open a new tab.",
      code: "page_crashed",
      retryable: true,
  ...
  if (
    isTimeoutError(err) &&
    err.code !== "tab_timeout" &&
    userId &&
    NAVIGATION_TIMEOUT_ACTIONS.has(action)
  ) {
  ...
  if (isTabDestroyedError(err)) {
    return res.status(410).json({
      error: "Tab was destroyed. Open a new tab.",
      code: "tab_destroyed",
      retryable: true,
      recovery: "create_new_tab",
      ...extraFields,
  ...
  if (isDeadContextError(err)) {
    return res.status(503).json({
      error: "Browser session expired. Retry to get a fresh session.",
...
async function destroyTimedOutTab(session, tabId, reason, userId) {
  ...
  log("warn", "destroying timed-out tab", {
    tabId,
    listItemId,
    toolCalls: tabState.toolCalls,
    reason,
...
function tabNotFoundResponse(res, tabId) {
  ...
  const uuidPart =
    tabId &&
    tabId.includes("_") &&
    !tabId.slice(0, tabId.indexOf("_")).includes("-")
      ? tabId.slice(tabId.indexOf("_") + 1)
      : tabId;
  if (
    _lastBrowserRestartAt &&
    Date.now() - _lastBrowserRestartAt < 300_000 &&
    UUID_RE.test(uuidPart) &&
    fly.isLocalTab(tabId)
  ) {
    return res.status(410).json({
...
async function camofoxPressureCleanup(options = {}) {
  ...
  if (!dryRun) {
    for (const item of selected) {
      ...
      sessionTabCounts.set(
        item.userId,
        Math.max(0, (sessionTabCounts.get(item.userId) || 0) - 1),
      );
      ...
      log("info", "tab reaped (pressure cleanup)", {
        userId: item.userId,
        tabId: item.tabId,
        listItemId: item.listItemId,
        idleMs: item.idleMs,
    ...
    for (const [userId, session] of Array.from(sessions.entries())) {
      for (const [listItemId, group] of Array.from(
        session.tabGroups.entries(),
      )) {
        if (group.size === 0) session.tabGroups.delete(listItemId);
      }
      if (
        closeEmptySessions &&
        session.tabGroups.size === 0 &&
        !hasActivePageLeases(session)
      ) {
        session._closing = true;
        await closeSession(userId, session, {
          reason: "pressure_cleanup_empty_session",
          clearDownloads: true,
          clearLocks: true,
        });
...
async function rotateGoogleTab(
  ...
  await withPageLoadDuration("navigate", () =>
    navigatePage(page, "https://www.google.com/"),
  );
  tabState.visitedUrls.add("https://www.google.com/");
  await page.waitForTimeout(1200);
  await withPageLoadDuration("navigate", () =>
    navigatePage(page, tabState.lastRequestedUrl),
  );
...
async function _buildRefsInner(page, refs, start) {
  ...
  if (iframeRemaining > 2000 && refCounter <= MAX_SNAPSHOT_NODES) {
    ...
    for (const frame of childFrames) {
      ...
      if (
        IFRAME_SKIP_PATTERNS.some((p) => p.test(frameUrl) || p.test(frameName))
      )
        continue;
      ...
      try {
        const frameYaml = await frame
          .locator("body")
          .ariaSnapshot({ timeout: IFRAME_SNAPSHOT_TIMEOUT_MS });
        ...
        log("debug", "buildRefs: processed iframe", {
          frameName,
        ...
        log("debug", "buildRefs: iframe snapshot failed", {
          frameName,
    ...
    if (iframesProcessed > 0) {
      log("info", "buildRefs: processed iframes", {
        count: iframesProcessed,
...
async function getAriaSnapshot(page) {
  ...
  for (const frame of childFrames) {
    ...
    // Skip tracking/analytics iframes
    if (IFRAME_SKIP_PATTERNS.some((p) => p.test(frameUrl) || p.test(frameName)))
      continue;
    ...
    try {
      ...
      if (!label) {
        try {
          label = new URL(frameUrl).hostname;
        } catch {
          label = "iframe";
        }
      }
  ...
  if (frameName || frameUrl) {
    ...
    if (!frame && frameUrl) {
      ...
      frame = page
        ...
        .find(
          (f) =>
            f.url() === frameUrl || f.url().startsWith(frameUrl.slice(0, 80)),
        );
    }
    if (frame) {
    ...
    log("warn", "refToLocator: frame not found for iframe ref", {
      ref,
      frameName,
...
app.get("/health", (req, res) => {
  ...
  if (
    !running &&
    _lastBrowserStopReason &&
    !INTENTIONAL_STOP_REASONS.has(_lastBrowserStopReason)
  ) {
  ...
  res.json({
    ...
    activeSessions: sessions.size,
    consecutiveFailures: Array.from(userNavHealth.values()).reduce(
      (sum, h) => sum + h.consecutiveNavFailures,
      0,
    ),
    memory: { rssMb, heapUsedMb, nativeMemMb },
...
app.post("/pressure/cleanup", async (req, res) => {
  try {
    ...
    log("error", "pressure cleanup failed", {
      reqId: req.reqId,
      error: err.message,
    });
...
app.post("/tabs", async (req, res) => {
  try {
    ...
    if (FLY_MACHINE_ID) {
      ...
      if (
        sessions.size >= PER_MACHINE_SESSION_CAP &&
        !sessions.has(key) &&
        !isReplayed
      ) {
    ...
    const result = await withTimeout(
      (async () => {
        ...
        const createdPage = await createPageWithRecoveryForUser(
          userId,
          session,
          { trace: !!trace },
        );
        ...
        if (url) {
          ...
          try {
            ...
            if (
              (isProxyError(navErr) || isTimeoutError(navErr)) &&
              proxyPool?.canRotateSessions
            ) {
              log(
                "warn",
                "tab create navigate failed, retrying with fresh proxy",
                {
                  reqId: req.reqId,
                  tabId,
              ...
              const { page: retryPage, lease: retryLease } =
                await createLeasedPage(session);
      ...
      withTimeout(
        (async () => {
          ...
          const resolvedSessionKey =
            sessionKey || listItemId || found.listItemId || "default";
            ...
            async () => {
              ...
              const navigateCurrentPage = async () => {
                ...
                const gotoP = withPageLoadDuration("navigate", () =>
                  navigatePage(tabState.page, targetUrl),
                );
                try {
                  await Promise.race([
                    gotoP,
              ...
              const prewarmGoogleHome = async () => {
                ...
                try {
                  await withPageLoadDuration("navigate", () =>
                    navigatePage(prewarmPage, "https://www.google.com/"),
                  );
                  tabState.visitedUrls.add("https://www.google.com/");
              ...
              try {
                ...
                if (
                  ...
                  log(
                    "warn",
                    "navigate failed, retrying with fresh proxy session",
                    {
                      reqId: req.reqId,
                      tabId,
                      error: navErr.message,
...
app.get("/tabs/:tabId/snapshot", async (req, res) => {
  try {
    ...
    if (!found)
      return tabNotFoundResponse(res, req.params.tabId || req.body?.tabId);
    session.lastAccess = Date.now();
    ...
    if (offset > 0 && tabState.lastSnapshot) {
      ...
      if (req.query.includeScreenshot === "true") {
        const data = await captureTabScreenshotBase64({
          userId,
          tabId: req.params.tabId,
          page: tabState.page,
        });
        response.screenshot = { data, mimeType: "image/png" };
      }
      ...
      withTabLock(
        ...
        async () => {
          ...
          const dispatchMouseSequence = async (locator) => {
            ...
            const bboxTimeout = Math.max(
              500,
              Math.min(3000, remainingBudget()),
            );
            let box;
            try {
              box = await locator.boundingBox({ timeout: bboxTimeout });
            } catch (e) {
              const detachedErr = new Error(
                `Element not actionable: no bounding box within ${bboxTimeout}ms (element likely detached after page change). Call snapshot to refresh refs and retry.`,
              );
            ...
            // Full click sequence
            await withTimeout(
              tabState.page.mouse.down(),
              Math.max(1, remainingBudget()),
              "native mouse down",
            );
            await tabState.page.waitForTimeout(50);
            await withTimeout(
              tabState.page.mouse.up(),
              Math.max(1, remainingBudget()),
              "native mouse up",
            );
          ...
          const doClick = async (locatorOrSelector, isLocator) => {
            ...
            const click = async (options) =>
              clickWithDownloadGuard(tabState, () => locator.click(options));
      ...
      try {
        ...
        if (found?.tabState?.page && !found.tabState.page.isClosed()) {
          ...
          return res.status(409).json({
            error:
              "Page changed during click. Call snapshot to see the current state and retry with current refs.",
            code: "page_changed",
            retryable: true,
...
app.post("/tabs/:tabId/upload", async (req, res) => {
  ...
  try {
    ...
    const uploadTimeout =
      Number.isFinite(req.body.timeout) && req.body.timeout > 0
        ? req.body.timeout
    ...
    if (!filePath)
      return res
        .status(400)
        .json({ error: "path required (container-side file path)" });

    const paths = await resolveUploadPaths({
      ...
      withTabLock(tabId, async () => {
          ...
          try {
            if ((await directInput.count()) > 0) {
              await directInput.setInputFiles(paths, {
                timeout: UPLOAD_INPUT_TIMEOUT_MS,
              });
              return true;
            }
          } catch (e) {
            log("info", "upload: setInputFiles on existing input failed", {
              error: e.message,
            });
          }
          return false;
        ...
        if (!attachedVia) {
          ...
          if (ref) {
            ...
            if (!locator) {
              try {
                tabState.refs = await refreshTabRefs(tabState, {
                  reason: "pre_upload",
                  timeoutMs: UPLOAD_REFS_TIMEOUT_MS,
                });
              } catch (e) {
                /* proceed without refresh */
              }
              locator = refToLocator(tabState.page, ref, tabState.refs);
            }
            if (!locator) {
            ...
            const err = new Error(
              "No input[type=file] present and no ref/selector trigger provided.",
            );
            err.statusCode = 400;
            throw err;
          }
          ...
          try {
            ...
            try {
              await locator.click({
                timeout: UPLOAD_CLICK_TIMEOUT_MS,
                force: true,
              });
            } catch (e2) {
              /* chooser/panel may still appear */
            }
          }
          ...
          if (!attachedVia) {
            ...
            if (fc) {
              try {
                ...
                log("info", "upload: setFiles on filechooser failed", {
                  error: e.message,
                });
              }
            }
          ...
          if (!attachedVia) {
            const err = new Error(
              "Upload trigger did not open a file chooser or mount a file input.",
            );
            err.statusCode = 422;
            throw err;
          }
    ...
    if (
      ...
      try {
        ...
        if (found?.tabState?.page && !found.tabState.page.isClosed()) {
          ...
          return res.status(409).json({
            error:
              "Page changed during type. Call snapshot to see the current state and retry with current refs.",
            code: "page_changed",
            retryable: true,
            recovery: "snapshot_then_retry",
            hint: "The page may have changed. Call snapshot to see the current state and retry.",
...
app.post("/tabs/:tabId/viewport", async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "width and height required (100..4000 px)" });
    }
    const session = sessions.get(normalizeUserId(userId));
    const found = session && findTab(session, req.params.tabId);
    ...
    await tabState.page.setViewportSize({
      width: Math.round(width),
      height: Math.round(height),
    });
    await tabState.page.waitForTimeout(150);

    pluginEvents.emit("tab:viewport", {
      userId,
      tabId: req.params.tabId,
      width,
      height,
...
app.get("/tabs/:tabId/screenshot", async (req, res) => {
  try {
    ...
    const { tabState } = found;
    const buffer = await captureTabScreenshot({
      userId,
      tabId: req.params.tabId,
      page: tabState.page,
      fullPage,
...
app.post(
  "/tabs/:tabId/evaluate",
  express.json({ limit: CONFIG.evaluateMaxBodySize }),
  async (req, res) => {
    try {
...
if (FLY_MACHINE_ID) {
  const MEMORY_HIGH_WATERMARK = 0.8;
  setInterval(() => {
...
setInterval(() => {
  ...
  if (reaped > 0)
    log("warn", "orphan page reaper closed leaked pages", { reaped });
}, 60_000);
...
setInterval(() => {
  ...
  if (
    browserRssMb !== null &&
    browserRssMb >= CONFIG.browserRssRestartThresholdMb
  ) {
    ...
    closeBrowserFully("browser_rss_pressure").catch((err) => {
      log("error", "browser rss pressure browser close failed", {
        error: err.message,
      });
    });
    return;
...
app.post("/navigate", async (req, res) => {
  try {
    ...
    const result = await withTabLock(targetId, async () => {
      await withPageLoadDuration("navigate", () =>
        navigatePage(tabState.page, url),
      );
      recordNavSuccess(userId);
      tabState.visitedUrls.add(url);
      tabState.lastSnapshot = null;
...
const server = app.listen(PORT, CONFIG.bindHost || undefined, async () => {
  ...
  const bindHost =
    typeof address === "object" && address ? address.address : CONFIG.bindHost;
  ...
  try {
    ...
    if (isFatalInstallError(err)) {
      log(
        "error",
        "browser pre-warm aborted: Camoufox binaries are not installed",
        {
          error: err.message,
          remediation: camoufoxInstallRemediation(),
        },
